...
from Shaheen import app,BOT_ID
from Shaheen.utils.filter_groups import cbot
from lang import get_command
from Shaheen.utils.lang import language
from Shaheen.mongo import chatb
from Shaheen.plugins.antlangs import get_arg
...
async def szcbot(_, message: Message):
    ...
    if chat:
        ...
        try:
           ...
           affiliateplus = requests.get(f"https://api.affiliateplus.xyz/api/chatbot?message={text}&botname=Shaheen&ownername=@supunma&user={chat_id}")
           textmsg = (affiliateplus.json()["message"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "Shaheen")
           ...
           trtoen = (message.text if lang=="en" else tr.translate(message.text, dest="en").text).replace(" ", "%20")
           text = trtoen.replace(" ", "%20") if len(message.text) < 2 else trtoen
           safeone = requests.get(f"https://api.safone.tech/chatbot?message={text}&bot_name=Shaheen&bot_master=Supun&user_id={user_id}")
